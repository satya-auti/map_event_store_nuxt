    ...
    <div class="left1 bg-gray">
      ...
      <div class="">
        ...
        <table class="">
          ...
          <tr v-for="(data, index) in info.allData" :key="data.name">
...
import Popup1 from "../components/popup.vue";
...
async function getMapData(map: mapboxgl.Map) {
  ...
  function showDataOnMap(i) {
    ...
    //   let b = getMapData(map);
    //   b.styleView(map);

    if (info.checkedInfo != null) {
      // getMapData.call(styleView(map));
      // getMapData().(styleView(map));
    }
